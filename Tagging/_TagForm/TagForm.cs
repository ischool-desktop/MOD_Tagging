...
namespace Tagging
{
    ...
    internal partial class TagForm : BaseForm
    {
        ...
        {
            List<TagConfigRecord> records = TagConfig.SelectByCategory(Category).Viewable().ToList();
        ...
        private void RefreshPrefixList()
        {
            ...
            foreach (TagConfigRecord each in TagConfig.SelectByCategory(Category).Viewable())
            {
        ...
        private void DGV_SelectionChanged(object sender, EventArgs e)
        {
            ...
            TagConfigRecord tag = DGV.SelectedRows[0].Tag as TagConfigRecord;

            if (tag == null)
                return;
            else
            {
                if (string.IsNullOrWhiteSpace(tag.AccessControlCode))
                    return;
                else
                {
                    btnDelete.Enabled = UserAcl.Current[tag.AccessControlCode].Editable;
                    btnModify.Enabled = UserAcl.Current[tag.AccessControlCode].Editable;
                }
            }
        }
    }
