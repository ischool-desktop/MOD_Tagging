...
namespace Tagging.BaseModel
{
    ...
    /// 
    /// </summary>
    internal partial class CourseView : ViewHelper.TreeNavViewBase
    {
        public CourseView()
            : base()
        {
            NavText = "類別檢視";
            ShowRoot = true;
            CourseTag.AfterDelete += (Tag_AfterDelete);
            CourseTag.AfterInsert += (Tag_AfterInsert);
            CourseTag.AfterUpdate += (Tag_AfterUpdate);
        ...
        protected override void GenerateTreeStruct(KeyCatalog root)
        {
            ...
            IEnumerable<CourseTagRecord> tagRecordList = CourseTag.SelectByCourseIDs(Source).Viewable();
            ISet<string> nocatalog = new HashSet<string>(Source);

            foreach (CourseTagRecord student in tagRecordList)
            {
