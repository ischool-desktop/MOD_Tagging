...
namespace Tagging.BaseModel
{
    ...
    /// 
    /// </summary>
    internal partial class TeacherView : ViewHelper.TreeNavViewBase
    {
        public TeacherView()
            : base()
        {
            NavText = "類別檢視";
            ShowRoot = true;
            TeacherTag.AfterDelete += (Tag_AfterDelete);
            TeacherTag.AfterInsert += (Tag_AfterInsert);
            TeacherTag.AfterUpdate += (Tag_AfterUpdate);
        ...
        protected override void GenerateTreeStruct(KeyCatalog root)
        {
            ...
            IEnumerable<TeacherTagRecord> tagRecordList = TeacherTag.SelectByTeacherIDs(Source).Viewable();
            ISet<string> nocatalog = new HashSet<string>(Source);

            foreach (TeacherTagRecord student in tagRecordList)
            {
