...
namespace Tagging
{
    class ImportStudentTag : SmartSchool.API.PlugIn.Import.Importer
    {
        ...
        public override void InitializeImport(SmartSchool.API.PlugIn.Import.ImportWizard wizard)
        {
            ...
            Dictionary<string, TagConfigRecord> PermissionMap = TagConfig.SelectByCategory(TagCategory.Student).ToDictionary(x => x.FullName);
            ...
            wizard.ValidateRow += delegate(object sender, SmartSchool.API.PlugIn.Import.ValidateRowEventArgs e)
            {
                ...
                string tagName = GetFullTagName(e.Data);

                if (PermissionMap.ContainsKey(tagName))
                {
                    TagConfigRecord tc = PermissionMap[tagName];
                    if (!string.IsNullOrWhiteSpace(tc.AccessControlCode))
                    {
                        if (!UserAcl.Current[tc.AccessControlCode].Editable)
                        {
                            e.ErrorMessage = "您沒有權限調整此類別的資料。";
                            return;
                        }
                    }
                }
        ...
        private string GetFullTagName(RowData rowData)
        {
            string prefix = ImportItemList[0]; //群組
            string name = ImportItemList[1]; //類別名稱

            if (string.IsNullOrWhiteSpace(rowData[prefix]))
                return string.Format("{0}", rowData[name]);
            else
                return string.Format("{0}:{1}", rowData[prefix], rowData[name]);
        }
